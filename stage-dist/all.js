var helper = {
    init: function(){
        if(!localStorage){
            delete helper.Storage.local;
        }
        if(!sessionStorage){
            delete helper.Storage.session;
        }
    },
    Objects: {
      timerID: 0
    ...
    Ajax: {
      ...
      GetReq: function (url, headers, cb) {
      ...
      PostReq: function (url, data, headers, cb) {
        var params;
        ...
        for(var prop in data){
            tmpArr.push(encodeURIComponent(prop)+'='+encodeURIComponent(data[prop]))
        }
        params = tmpArr.join('&');
        }
    ...
    Event: {
        ...
        remove: function (eventName, fn) {
            if (helper.Event.events[eventName]) {
                var len = helper.Event.events[eventName];
            for (var i = 0; i < len; i++) {
    ...
    WebSocket: {
        ...
        keepAlive: function () {
          if (helper.WebSocket.ws) {
            if (helper.WebSocket.ws.readyState === helper.WebSocket.state.OPEN) {
                helper.WebSocket.ws.send('');
            }
            helper.Objects.timerID = setTimeout(helper.WebSocket.keepAlive, helper.WebSocket.webSocketTimeout);
          }
        },
        getState: function () {
